...
//For Product Invoice
describe("Test case for ProductInvoice", () => {
  test("Test to find the title", async () => {
    render(
      <HashRouter>
        <NewInvoice />
      </HashRouter>
    );
    const newInvoice = await screen.findAllByText(/Summary/);
    expect(newInvoice[0]).toBeInTheDocument();
  })

  test('To find text total balance', async () => {
    render(
      <HashRouter>
        <NewInvoice />
      </HashRouter>
    );
    const Total_Balance = await screen.findAllByText('Total Balance');
    expect(Total_Balance[0]).toBeInTheDocument();
  })

  test('Test for add new From', async () => {
    render(
      <HashRouter>
        <NewInvoice />
      </HashRouter>
    );
    const AddNewItem = await screen.findByTestId("Add-New-Item");
    expect(AddNewItem).toBeInTheDocument();
    fireEvent.click(AddNewItem)
  })

  test('Test remove item button', async () => {
    render(
      <HashRouter>
        <NewInvoice />
      </HashRouter>
