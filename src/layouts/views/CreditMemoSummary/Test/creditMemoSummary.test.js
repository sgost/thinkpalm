...
describe("View change log", () => {
    ...
    test("Render change log", async () => {
        ...
        const input = await waitFor(() =>
            screen.getByText(/View Change Log/)
        );
        
        fireEvent.click(input);
        const publish = screen.getByText(/View More/);
        ...
        fireEvent.click(publish);
        //fetch view less
        const viewLess = screen.getByText(/View Less/);
        expect(viewLess).toBeInTheDocument();
        expect(viewLess).toBeEnabled();
