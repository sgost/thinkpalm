
import { fireEvent, render, screen, waitFor } from "@testing-library/react";
import { HashRouter } from "react-router-dom";
import NotesWidget from "../index";
import MockAdapter from "axios-mock-adapter";
import axios from "axios";
import { saveEditNoteApi, getHeaders } from '../../../urls/urls'
import { creditMemoData, editApiMock, noteDeleteApi } from "./mockData"

const id = "e6adaad5-089b-4a6f-b24f-a398b33c8cf3";
const cid = "a9bbee6d-797a-4724-a86a-5b1a2e28763f";
var token = "eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICIwdmRELXE3ekFYdkFxUzRfTDdoUExua2ZJbVVzaW1NWE1ZWGoxVUYwUUxVIn0..kQwAyl4f2lOXpiCysnYmZNLs3qUE7zkJ2xozWExr55W_pdBCcXV2nDVf0TsAh022NJ0xr4Mx7hxOkFhJKatg42IvxljZoxunHv88QqKYpGMK_r-B1YqSR6Vz1hUwOSaF_zLFlseR292icQWmxuNXRvtoUfN4shurHNIob63Dua5sNLfBEJL0BF4xDNtgQrn6H5SImOm8274a8J1BK4StteS0GOAbtZyrAn1h1N5xLb0q-RykXru4PSlHOuS_vCjnX5ZIX0zW4gEuuNqzfcSjjrB5hZRRprxjrbVJXGn-XvxIUULXSj0jPcO7w8j3iIGkTjnqzNAtd5iu5_2UUgU2bw"
localStorage.setItem("accessToken", "eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICIwdmRELXE3ekFYdkFxUzRfTDdoUExua2ZJbVVzaW1NWE1ZWGoxVUYwUUxVIn0..kQwAyl4f2lOXpiCysnYmZNLs3qUE7zkJ2xozWExr55W_pdBCcXV2nDVf0TsAh022NJ0xr4Mx7hxOkFhJKatg42IvxljZoxunHv88QqKYpGMK_r-B1YqSR6Vz1hUwOSaF_zLFlseR292icQWmxuNXRvtoUfN4shurHNIob63Dua5sNLfBEJL0BF4xDNtgQrn6H5SImOm8274a8J1BK4StteS0GOAbtZyrAn1h1N5xLb0q-RykXru4PSlHOuS_vCjnX5ZIX0zW4gEuuNqzfcSjjrB5hZRRprxjrbVJXGn-XvxIUULXSj0jPcO7w8j3iIGkTjnqzNAtd5iu5_2UUgU2bw");
localStorage.setItem("current-org-id", "E291C9F0-2476-4238-85CB-7AFECDD085E4");
const noteIndex = 41077;


const noteData = [
    {
        "invoiceId": "03f0e8d3-abed-4835-8d51-7ada2e92acda",
        "noteType": 2,
        "note": "rrr",
        "isCustomerVisible": false,
        "exportToQuickbooks": false,
        "createdDate": "2022-07-05T07:04:30.347Z",
        "modifiedBy": "28a34839-4798-4faa-9786-0677e1680f22",
        "modifiedByUser": {
            "userName": null,
            "normalizedUserName": null,
            "email": null,
            "normalizedEmail": null,
            "emailConfirmed": false,
            "firstName": null,
            "lastName": null,
            "passwordHash": null,
            "securityStamp": null,
            "concurrencyStamp": null,
            "lockoutEnabled": false,
            "phoneNumber": null,
            "phoneNumberConfirmed": false,
            "twoFactorEnabled": false,
            "lockoutEnd": null,
            "accessFailedCount": 0,
            "isExternal": false,
            "customerUsers": [],
            "document": null,
            "id": "00000000-0000-0000-0000-000000000000"
        },
        "displayInPDF": false,
        "customerId": "a9bbee6d-797a-4724-a86a-5b1a2e28763f",
        "declineOption": null,
        "id": 41135
    }
]

describe("Credit Memo Summary", () => {
    beforeAll(() => {
        var temp = localStorage.getItem('accessToken');
        const mock = new MockAdapter(axios);
        mock.onPut(saveEditNoteApi(noteIndex), editApiMock, getHeaders(temp, cid, id)).reply(200, editApiMock);
        mock.onDelete(saveEditNoteApi(noteIndex), getHeaders(temp, cid, id)).reply(200, noteDeleteApi);
    })
    test("Render Notes", () => {
        render(
            <HashRouter>
                <NotesWidget
                    status='Open'
                    notes={noteData}
                    setNotes={() => { }}
                    isClient={false}
                    cid={cid}
                    id={id}
                    transactionType={2}
                    creditMemoData={creditMemoData}
                ></NotesWidget>
            </HashRouter>
        );
    })
})
describe("Notes fail", () => {
    beforeAll(() => {
        var temp = localStorage.getItem('accessToken');
        const mock = new MockAdapter(axios);
        mock.onPut(saveEditNoteApi(noteIndex), editApiMock, getHeaders(temp, cid, id)).reply(200, editApiMock);
        mock.onDelete(saveEditNoteApi(noteIndex), getHeaders(temp, cid, id)).reply(200, noteDeleteApi);
    })
    test("Render CM", async () => {
        const { getByTestId, getByText } = render(
            <HashRouter>
                <NotesWidget
                    status='Open'
                    notes={noteData}
                    setNotes={() => { }}
                    isClient={false}
                    cid={cid}
                    id={id}
                    transactionType={2}
                    creditMemoData={creditMemoData}
                ></NotesWidget>
            </HashRouter>
        );
        const input = await waitFor(() =>
            screen.getByPlaceholderText(/Add a note here.../)
        );

        fireEvent.change(input, { target: { value: "" } });
        const publish = screen.getByText(/Save/);
        fireEvent.click(publish);


        var editButton = getByTestId("note-edit")
        fireEvent.click(editButton);

        var saveButton = getByTestId("note-save")
        fireEvent.click(saveButton);

        var deleteButton = getByTestId("note-delete")
        fireEvent.click(deleteButton);

        fireEvent.click(editButton);
        const editInput = await waitFor(() =>
            screen.getByTestId("note-input")
        );
        fireEvent.change(editInput, { target: { value: "" } });
    });
})
